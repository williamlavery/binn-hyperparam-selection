"""Prepare diffusion MSE summaries from trained model wrappers.

Contents
--------
- _trained_epoch_count
- _diff_prediction_count
- infer_diff_prediction_frequency
- _central_90_mask
...
def _diff_prediction_count(model_wrapper):
    preds = getattr(model_wrapper, "diffusion_preds", None)
    if preds is not None and len(preds) > 0:
        return len(preds)

    errors = getattr(model_wrapper, "diffusion_errors", None)
...
def infer_diff_prediction_frequency(model_wrappers):
    """Infer saved-diffusion-prediction cadence from stored arrays and epochs."""
    freqs = []
    for model_wrapper in model_wrappers:
        prediction_count = _diff_prediction_count(model_wrapper)
...
def prepare_diff_run_data(
    ...
    for i, (key, model_dic) in enumerate(zip(model_keys, dic_values)):
        ...
        sf = infer_diff_prediction_frequency(model_wrappers)
        ...
        for m in model_wrappers:
            ...
            if restrict_to_central_90 and getattr(m, "diffusion_preds", None):
                true_vals = getattr(m.model, "D_true_torch", None)
                if true_vals is None:
                    raise AttributeError(
                        "restrict_to_central_90=True requires model.D_true_torch to be available."
                    )
                true_vals = true_vals.detach().cpu().numpy() if torch is not None and isinstance(true_vals, torch.Tensor) else true_vals
                for diff_pred in m.diffusion_preds:
                    pred_vals = diff_pred.detach().cpu().numpy() if torch is not None and isinstance(diff_pred, torch.Tensor) else diff_pred
        ...
        if restrict_to_central_90:
            final_mses = []
            for m in model_wrappers:
                best_pred = getattr(m, "best_diffusion_pred", None)
                true_vals = getattr(m.model, "D_true_torch", None)
                mask = _central_90_mask(m, dataobj=dataobj)
                if best_pred is None or true_vals is None:
                    raise AttributeError(
                        "restrict_to_central_90=True requires best_diffusion_pred and model.D_true_torch."
                    )
