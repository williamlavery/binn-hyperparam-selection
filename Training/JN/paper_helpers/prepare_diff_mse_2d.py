"""Prepare two-dimensional diffusion MSE summaries from trained wrappers.
...
def prepare_diff_run_data(
    ...
    for i, (key, model_dic) in enumerate(zip(model_keys, dic_values)):
        ...
        for m in model_wrappers:
            ...
            if restrict_to_central_90 and getattr(m, "diffusion_preds", None):
                ...
                for diff_pred in m.diffusion_preds:
                    pred_vals = (
                        diff_pred.detach().cpu().numpy()
                        if torch is not None and isinstance(diff_pred, torch.Tensor)
                        else diff_pred
                    )
        ...
        if val_loss_best:
            ...
            best_seed_idx = np.nanargmin(diff_errs_arr[:, best_epoch_idx])
        ...
        if restrict_to_central_90:
            ...
            final_mses = np.array(
                [m.best_diffusion_error for m in model_wrappers], dtype=np.float64
            )
